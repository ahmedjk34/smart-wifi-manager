...
wifi.SetRemoteStationManager("ns3::SmartWifiManagerV3Logged", 
    "LogFilePath", StringValue("logged-results/" + tc.scenarioName + ".csv"));
    WifiMacHelper mac;
    ...
    onoff.SetAttribute("StopTime", TimeValue(Seconds(38.0))); // increased run time
    ...
    serverApps.Stop(Seconds(40.0)); // increased run time
    ...
    for (uint32_t i = 0; i < tc.numInterferers; ++i)
    {
        ...
        interfererOnOff.SetAttribute("StopTime", TimeValue(Seconds(38.0)));
    ...
    double rxBytes = 0;
    double simulationTime = 36.0; // from 2s to 38s
...
int main(int argc, char *argv[])
{
    ...
    std::vector<double> distances = { 1.0, 20.0, 40.0, 80.0, 120.0 };         // 5
    std::vector<double> speeds = { 0.0, 2.0, 5.0, 10.0, 20.0 };               // 5
    std::vector<uint32_t> interferers = { 0, 3 };                             // 2
    std::vector<uint32_t> packetSizes = { 256, 1500 };                        // 2
    std::vector<std::string> trafficRates = { "11Mbps", "54Mbps" };           // 2
    ...
    std::ofstream csv("smartv3-benchmark-logged.csv");
    ...
    csv.close();
    std::cout << "All tests complete. Results in smartv3-benchmark-logged.csv\n";
    return 0;
}